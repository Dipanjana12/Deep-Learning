...
from Helper_Function import map_nested_dict, MissingInput
...
class LSTMCell:  # todo: uncouple the linear classifier later
    def __init__(self, input_dim, output_dim, pc):
        ...
        for gate in self.gate_names:
            ...
            self.parameters["W"]["h"][gate] = self.pc.add_parameters(
                (self.output_dim, self.output_dim))  # takes its own previous output
...
class LSTMNetworkState:
    def __init__(self, cs=None, hs=None, full_vec=None, hidden_dim=None):
        if not None in [full_vec, hidden_dim]:
            ...
            self.cs = [dy.inputVector(cvec[i * hidden_dim:(i + 1) * hidden_dim]) for i in
                       range(int(length / hidden_dim))]
            self.hs = [dy.inputVector(hvec[i * hidden_dim:(i + 1) * hidden_dim]) for i in
                       range(int(length / hidden_dim))]
